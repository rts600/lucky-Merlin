...
func status(c *gin.Context) {
	...
	currentProcessMem := m.Alloc
	
	appInfo := config.GetAppInfo()

	respMap := make(map[string]interface{})
	respMap["totleMem"] = stringsp.BinaryUnitToStr(m.Sys)
	respMap["usedMem"] = stringsp.BinaryUnitToStr(m.Alloc)
	respMap["unusedMem"] = stringsp.BinaryUnitToStr(m.Sys - m.Alloc)
	respMap["currentProcessUsedCPU"] = "0.00%"
